...
class UserService {
    ...
    static func fetchUserStats(uid: String) async throws -> UserStats {
        ...
        async let postSnapshot = try await FirebaseConstants.PostCollection.whereField("ownerUid", isEqualTo: uid).getDocuments()
        let postCount = try await postSnapshot.count
        
        print("DEBUG: Did call fetch user stats...")
        return .init(followingCount: followingCount, followersCount: followersCount, postCount: postCount)
    }
}
